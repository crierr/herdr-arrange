...
- **`e`** → `tree.Balance`: tmux's `select-layout -E`. Keep the layout exactly as it is and
  re-weight every split so the cells either side of it get the same room along that split's
  own axis — a child that splits the other way counts as one cell, so `[[a | b] | [c / d]]`
  becomes three columns of equal width. Pure `layout.set_split_ratio` calls: non-destructive,
  no flicker, no pane moves, and it leaves `main-*` main panes at their half of the tab.
  (Caveat: herdr clamps ratios to [0.1, 0.9], so a right-nested run of >10 cells cannot be
  evened out exactly; the UI says so. The presets avoid this by building balanced trees,
  see §5.)
